...
```{r echo=FALSE}
maxInterval <- meanByInterval[V1 == max(meanByInterval$V1),]
```
- The 5 minute interval with the most steps is `r maxInterval$interval ` having `r maxInterval$V1 ` steps on average.
...
```
- The number of rows with missing data is `r sum(missingRows) `
